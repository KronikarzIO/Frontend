...
treeNameInput.addEventListener("focusout", (e) => {
  ...
  if (name.length > 0) {
    Api.patchFamilyTreeById(familyTreeId, { name: name });
  }
});
...
exportToPDF.addEventListener("click", () => {
  ...
  var svg = `
    <svg height="${bottom - up + drawableElements[0].height}" width="${
    right - left + drawableElements[0].width
  }">
    <style>
  ...
  drawableElements.forEach((e) => {
    svg += `
        <rect x="${e.x - left}" y="${e.y - up}" rx="15" ry="15" width="${e.width}" height="${
      e.height
    }"
  ...
  var doc = new PDFDocument({
    size: [
      (right - left + drawableElements[0].width) * 0.75,
      (bottom - up + drawableElements[0].height) * 0.75,
    ],
  });
  const chunks = [];
  ...
  drawableElements.forEach((e) => {
    doc.image(getBase64Image(e.imagePath), (e.x - left + 10) * 0.75, (e.y - up + 10) * 0.75, {
      fit: [100, 100],
    });
  });
